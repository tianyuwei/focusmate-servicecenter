package com.focusmate.services.wxpay.utils;
...
public class HttpsRequest {
    ...
    private void init() throws IOException, KeyStoreException, UnrecoverableKeyException, NoSuchAlgorithmException,
        ...
        FileInputStream instream = new FileInputStream(new File(HttpsRequest.class.getClassLoader().getResource(WxPayConfigure.getCertLocalPath()).getPath()));// 加载本地的证书进行https加密传输
        try {
            keyStore.load(instream, WxPayConfigure.getCertPassword().toCharArray());// 设置证书密码
        ...
        SSLContext sslcontext = SSLContexts.custom()
                .loadKeyMaterial(keyStore, WxPayConfigure.getCertPassword().toCharArray()).build();
    ...
    public String sendPost(String url, String xmlObj) throws IOException, KeyStoreException, UnrecoverableKeyException,
